...
void RenX_SetJoinPlugin::RenX_OnJoin(RenX::Server *server, const RenX::PlayerInfo *player)
{
	if (player->steamid != 0)
	{
		const Jupiter::ReadableString &setjoin = Jupiter::IRC::Client::Config->get(configSection, Jupiter::StringS::Format("0x%.16llX", player->steamid));
...
void DelJoinGameCommand::trigger(RenX::Server *source, RenX::PlayerInfo *player, const Jupiter::ReadableString &)
{
	if (player->steamid != 0)
	{
		if (Jupiter::IRC::Client::Config->remove(configSection, Jupiter::StringS::Format("0x%.16llX", player->steamid)))
...
void SetJoinGameCommand::trigger(RenX::Server *source, RenX::PlayerInfo *player, const Jupiter::ReadableString &parameters)
{
	if (player->steamid != 0)
	{
		...
		{
			Jupiter::IRC::Client::Config->set(configSection, Jupiter::StringS::Format("0x%.16llX", player->steamid), parameters);
